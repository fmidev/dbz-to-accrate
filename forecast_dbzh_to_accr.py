...
def run(timestamp, config):
    ...
    second_timestamp = (
        timestamp_formatted + datetime.timedelta(minutes=first_timestep)
    ).strftime("%Y%m%d%H%M%S")
    first_file = (
        input_conf["dir"]
        + "/"
        + input_conf["filename"].format(
            timestamp=f"{timestamp}00", fc_timestamp=second_timestamp, config=config
        )
    )
    (
        first_image_array,
    ...
    snowprob_file = (
        snowprob_conf["dir"]
        + "/"
        + snowprob_conf["filename"].format(
            timestamp=timestamp_formatted.strftime("%Y%m%d%H%M")
        )
    )
    (
        snowprob,
    ...
    lut_rr, lut_sr = dbzh_to_rate.calc_lookuptables_dBZtoRATE(
        input_conf["timeres"], coef, nodata, undetect, gain, offset
    )
    ...
    for timestep in range(
        input_conf["timeres"], input_conf["fc_len"] + 1, input_conf["timeres"]
    ):
        # Input file
        second_timestamp = (
            timestamp_formatted + datetime.timedelta(minutes=timestep)
        ).strftime("%Y%m%d%H%M%S")
        input_file = (
            input_conf["dir"]
            + "/"
        ...
        if output_conf["write_acrr_fixed_step"]:
            acc_rate_fixed_timestep = np.where(
                np.isnan(acc_rate_fixed_timestep),
                image_array,
                acc_rate_fixed_timestep + np.nan_to_num(image_array),
            )
            ...
            if timestep % output_conf["timestep"] == 0:
                ...
                write_acc_rate_fixed_timestep = utils.convert_dtype(
                    write_acc_rate_fixed_timestep,
                    output_conf,
                    nodata_mask,
                    undetect_mask,
                )
                ...
                outfile = (
                    output_conf["dir"]
                    + "/"
                    + output_conf["filename"].format(
                        timestamp=timestamp,
                        fc_timestamp=second_timestamp[0:12],
                        acc_timestep=f'{output_conf["timestep"]:03}',
                        config=config,
                    )
                )
        ...
        # Calculate sum and write to file in fixed time interval
        if output_conf["write_acrr_fixed_step_2"]:
            acc_rate_fixed_timestep_2 = np.where(
                np.isnan(acc_rate_fixed_timestep_2),
                image_array,
                acc_rate_fixed_timestep_2 + np.nan_to_num(image_array),
            )
            ...
            if timestep % output_conf["timestep_2"] == 0:
                ...
                write_acc_rate_fixed_timestep_2 = utils.convert_dtype(
                    write_acc_rate_fixed_timestep_2,
                    output_conf,
                    nodata_mask,
                    undetect_mask,
                )
                ...
                outfile = (
                    ...
                    + output_conf["filename"].format(
                        timestamp=timestamp,
                        fc_timestamp=second_timestamp[0:12],
                        acc_timestep=f'{output_conf["timestep_2"]:03}',
                        config=config,
                    )
                )
                enddate = fc_timestamp[0:8]
                ...
                # Init next sum array
                acc_rate_fixed_timestep_2 = np.full_like(
                    acc_rate_fixed_timestep_2, np.nan
                )
                n_fixed_2 = 0

        # Calculate and write to file accumulated rate from beginning of forecast
        if output_conf["write_acrr_from_start"]:
            acc_rate_from_start = np.where(
                np.isnan(acc_rate_from_start),
                image_array,
                acc_rate_from_start + np.nan_to_num(image_array),
            )

            if timestep % output_conf["timestep"] == 0:
                # Change nodata and undetect values from undetect = 0 and nodata = np.nan to undetect = -0.0001 and nodata = 0 before
                # writing to file
                nodata_mask = ~np.isfinite(
                    acc_rate_from_start
                )  # (acc_rate_from_start == np.nan)
                undetect_mask = acc_rate_from_start == 0
                write_acc_rate_from_start = acc_rate_from_start

                write_acc_rate_from_start = utils.convert_dtype(
                    write_acc_rate_from_start, output_conf, nodata_mask, undetect_mask
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--timestamp", type=str, default="2022011707", help="Input timestamp"
    )
    parser.add_argument(
        "--config", type=str, default="ravake", help="Config file to use."
    )
